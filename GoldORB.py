...
class goldstrategy:
    def __init__(self, symbol='GC=F', risk_reward_ratio=6):
        self.symbol = symbol
    ...
    def identify_london_session_start(self, date):
        london_open = self.london_tz.localize(datetime.combine(date, time(23, 0))).astimezone(pytz.UTC)
        return london_open
    
    def calculate_london_rectangle(self, data, date):
        london_start = self.identify_london_session_start(date)
        london_end = london_start + timedelta(minutes=14)
...
def main():
    strategy = goldstrategy(symbol='GC=F', risk_reward_ratio=6.0)
    end_date = datetime.now()
    start_date = end_date - timedelta(days=7)
    print("Stratégie GF")
    print(f"Période : {start_date.date()} à {end_date.date()}")
    print(f"Symbole : {strategy.symbol}")
    print(f"R/R : {strategy.risk_reward_ratio}")
    print(f"Timeframe : 1min")
    print(f"Session : Paris 0h-0h15 GMT\n")
